...
1. 如果选项适合度评分 >0.7，分析支持该选项的原因；如果选项适合度评分 < 0.3，分析反对该选项的原因；  
2. 指出可能的风险与注意事项；  
3. 分析该选项与正确答案的匹配度。  
...
"""
    ...
    def _build_treatment_reasoning_prompt_medqa(
        ...
        ==============================
        【任务要求】
        请从 **{role.value}** 专业角度对选项适合度进行评分：
        1. 为每个选项进行适合度评分，范围在-1到1之间，必须在这个范围内，-1表示完全不适合，1表示完全适合，0表示中性。  
        2. 指出可能的风险与注意事项；  
        3. 分析该选项与正确答案的匹配度。  
        ...
        ==============================
        【格式与一致性要求】
        - 仅返回 JSON，要干净的JSON格式，不得包含任何解释或额外文字；
        - treatment_preferences 的键必须是选项索引（如 \"A\"、\"B\"），不要使用选项全文。
