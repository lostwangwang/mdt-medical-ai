...
class MultiAgentDialogueManager:
    ...
    def conduct_mdt_discussion(
        ...
        while self.current_round < self.max_rounds:
            ...
            # 基于对话内容更新各角色立场
            self._update_agent_opinions(current_round, opinions_list)
    ...
    def _check_discussion_convergence(self, opinions_list: List[RoleOpinion]) -> bool:
        """
        检查讨论是否收敛
    ...
    def _update_agent_opinions(
        self, current_round: DialogueRound, opinions_list: List[RoleOpinion]
    ) -> List[RoleOpinion]:
        """
        更新各角色立场
        - 基于当前轮对话内容，更新每个角色的治疗意见
        ...
        for role, agent in self.agents.items():
            ...
            new_opintion = agent.update_agent_opinions_and_preferences(current_round, options_list)
            new_opinions_list.append(new_opinion)
