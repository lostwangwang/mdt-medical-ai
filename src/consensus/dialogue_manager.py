...
from ..core.data_models import (
    ...
    DialogueRound,
    ConsensusResult,
    RoleOpinion, QuestionOpinion, RoleRegistry,
)
...
class MultiAgentDialogueManager:
    ...
    def conduct_mdt_discussion_medqa(
        ...
        if self.consensus_bool == True:
            ...
            while self.current_round <= self.max_rounds and not self.consensus_bool:
                ...
                # 2. 进行一轮结构化对话
                current_round_messages = self._conduct_dialogue_round_medqa(
                    question_state, question_options, opinions_dict, mdt_leader_summary, dataset_name, self.current_round
                )
    ...
    def _conduct_dialogue_round_medqa(
        ...
        current_round_resp = self._conduct_sequential_presentation_medqa(
            current_round,
            question_state,
            question_options,
            opinions_dict,
        ...
        logger.info(
            f"Generated messages for current_round_resp: {self.current_round}: {current_round_resp}"
        )
        return current_round_resp
    ...
    def _initialize_discussion_medqa(
        ...
        for role, agent in self.agents.items():
            ...
            print(f"Generated initial message for {role.valuee}: {initial_message.content}")
    ...
    def _generate_llm_initial_message_meqa(
        ...
        try:
            # 对所有选项生成推理
            if hasattr(agent.llm_interface, "generate_all_treatment_reasoning_meqa"):
                reasoning = agent.llm_interface.generate_all_treatment_reasoning_meqa(
                    question_state=question_state,
                    role=agent.role,
                    opinion=opinion,
                    question_options=question_options,
                ...
                if reasoning and len(reasoning.strip()) > 0:
                    logger.debug(
                        f"Generated LLM initial message for {agent.role.value}: {reasoning}"
                    )
                    return reasoning
    ...
    def _select_focus_question_for_role_medqa(
        ...
        # 2. 找到最高得分
        max_score = max(
            prefs[option.name] for option in question_options if option.name in prefs
        )
        ...
        logger.info(
            f"选中的选项: {focus_question.name} ({question_state.options[focus_question.name]})"
        )  # 打印选项标识和内容
    ...
    def _check_discussion_convergence_medqa(
        ...
        logger.info(f"第{self.current_round}轮W: {self.W}, {type(self.W)}")
    ...
    def _update_agent_opinions_medqa(
        ...
        for role, agent in self.agents.items():
            ...
            logger.info(f"Previous opinion for {role}: {previous_opinion}")
